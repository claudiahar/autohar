            ...
            <>
                  ...
                  {items.map((item) => (
                    <div key={item.variantId} className="flex gap-4 p-3 rounded-xl bg-secondary/30">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="font-semibold text-primary mt-1">{parseFloat(item.price.amount).toFixed(0)} {item.price.currencyCode}</p>
                      </div>
              ...
              <div className="flex-shrink-0 space-y-4 pt-4 border-t bg-background">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-xl font-bold text-primary">{totalPrice.toFixed(0)} {items[0]?.price.currencyCode || 'RON'}</span>
                </div>
                <Button onClick={handleCheckout} className="w-full" size="lg" disabled={items.length === 0 || isLoading || isSyncing}>
                  {isLoading || isSyncing ? <Loader2 className="w-4 h-4 animate-spin" /> : <><ExternalLink className="w-4 h-4 mr-2" />Finalizează comanda</>}
                </Button>
              </div>
            </>
          )}
